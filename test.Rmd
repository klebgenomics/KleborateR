...
barnards_adj_site_sums <- barnards_adj_site %>% select(Site, raw_sum, adj_sum) %>% unique()
  
barnards_adj_site_Nigeria <- barnards_adj_site_Nigeria %>%
  select(-c(raw_sum, adj_sum)) %>%
  complete(Site, K_locus, Country, fill=list(raw_count=0, adj_count=0, raw_prop=0, adj_prop=0)) %>%
  left_join(barnards_adj_site_sums)

barnards_adj_site_Nigeria %>% 
  filter(K_locus=="KL116") %>% 
  metaprop(event=raw_count, n=raw_sum, studlab=Site,  
  method = "GLMM", sm = "PLOGIT", fixed = FALSE, random = TRUE, method.random.ci = TRUE)
...
# note we can plot the estimates also
barnards_adj_site_Nigeria %>% 
  filter(K_locus=="KL62") %>% 
  metaprop(event=raw_count, n=raw_sum, studlab=Site,  
  method = "GLMM", sm = "PLOGIT", fixed = FALSE, random = TRUE, method.random.ci = TRUE) %>%
...
# this layout prints the N values as well as estimates and CIs
barnards_adj %>% 
  filter(K_locus=="KL62") %>% 
